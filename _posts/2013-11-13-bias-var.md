...
The fisrt term in this equation is the square of the bias and the second term is the variance of the variable $$Y\vert x$$, which is a property of the distribution and thus totally out of our control.
...
The first term represents the variance of our prediction over different training sets $$D$$. The second term represents the bias, i.e., how well our prediction approximates the true $$Y$$. The first two terms make the **approximation error** together. The third term is the variance of $$Y$$, i.e. the random noise.
...
The first term is the variance of our learner, as $$E_{D}[\hat{h}(X \vert D)] = h^*(X)$$. The second term is actually the combination of the bias and noise and also reppresent the lowest possible MSE on future sample, as $$h^*(X) = argmin_{h \in H} E_{X,Y}[(h(X) - Y)^2]$$.
...
Here the first term is variance of the leaner for future sample, as said above; the second term represents the variance of the learner on training dataset; and the third term indicates the leaner's $$MSE$$ on training set, i.e. the expected training error. So we can easily conclude that for a learner, **the expected test error $$\geq$$ optimal test error $$\geq$$ expected training error**
